...
#vislabwwy: add a function to save out flow field
def save_flow_field(flow, save_path, count):
    # flow shape [B, 2, H, W] 
    flow = flow[0].permute(1, 2, 0).cpu().numpy()  # reshape to (H, W, 2)
    
    flow_x = flow[:, :, 0]  # optic flow in x-direction
    flow_y = flow[:, :, 1]  # optic flow in x-direction

    # Save img for visualization
    flow_magnitude = np.sqrt(flow_x ** 2 + flow_y ** 2)
    flow_magnitude = np.uint8(255 * (flow_magnitude - flow_magnitude.min()) / (flow_magnitude.max() - flow_magnitude.min()))
    flow_rgb = flow_viz.flow_to_image(flow)
    imwrite(os.path.join(save_path, f'flow_field_visual_{count}.tif'), flow_rgb)
    
    # Save flow_filed parameters 
    flow_x_normalized = np.uint8(255 * (flow_x - flow_x.min()) / (flow_x.max() - flow_x.min()))
    flow_y_normalized = np.uint8(255 * (flow_y - flow_y.min()) / (flow_y.max() - flow_y.min()))
    flow_field = np.stack([flow_x_normalized, flow_y_normalized], axis=-1)
    imwrite(os.path.join(save_path, f'flow_field_values_{count}.tif'), flow_field)
...
def process_pair(wf_img, gt_img, save_wf_path, save_gt_path, sup_wf_img=None, patch_size=256, stride=224, model=None,
    ...
    while x + patch_size < x_end:
        ...
        while y + patch_size < y_end:
            ...
            save_flow_field(flow_up, save_wf_path, count) #vislabwwy: save out flow field
